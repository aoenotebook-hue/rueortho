...
**The `rueortho.vercel.app` part of this step is history.** That address has
308-redirected to `easybone.org` since 2026-09-23, so the property worth
having now is the one for `easybone.org`, below. An existing
`rueortho.vercel.app` property can stay while Google follows the redirect.

`rueortho.vercel.app` was **verified in code**, two ways, so this step
...
**For `easybone.org`, which is the address now:** add it in Search Console
— as a **Domain** property, adding the DNS TXT record Google gives you in
Vercel → **Domains** → `easybone.org` → DNS records, since Vercel runs its DNS
(this covers `www.` and `http://` too), otherwise as a URL-prefix
property `https://easybone.org` — and submit
`https://easybone.org/sitemap-index.xml`. That file is generated by the build;
**do not upload a `sitemap.xml` or `sitemap.ts` by hand** — the ones uploaded
on 2026-09-23 were Next.js templates, which this Astro site cannot run, and
they turned CI red until they were removed.
