...
2. **Both addresses are real.** `contactEmail` in `src/data/site.ts` is
   ...
   `origin` to `https://easybone.org`, the author's domain (it was
   `https://rueortho.vercel.app` until 2026-09-23), and
...
**`easybone.org` did not need this section.** It was bought through Vercel on
2026-09-23 and uses Vercel's own DNS, so attaching it — and later
`www.easybone.org` — was a matter of adding the name under **Project →
Settings → Domains**; Vercel wrote the records itself. What follows is for a
domain registered somewhere else. The list of domains attached today, and
where each one redirects, is under "The domains" in `CLAUDE.md`.

The steps below assume a domain registered at WordPress.com but the site runs on Vercel, so only
...
The address to register is **`easybone.org`**. Ownership is already proved in
code, two ways, and both move with the site whatever its address:

- `public/googlef9618cb9fa32a0a8.html` — Google's own verification file, served
  at `https://easybone.org/googlef9618cb9fa32a0a8.html`.
...
1. [Google Search Console](https://search.google.com/search-console): add
   `easybone.org`. Either:
   - as a **Domain** property — Google gives you a DNS TXT record, which goes
     in Vercel → **Domains** → `easybone.org` → DNS records, since Vercel runs
     that domain's DNS. This one property covers `www.` and `http://` too; or
   - as a **URL prefix** property, `https://easybone.org`, verified by either
     method above with no DNS change.

   Then submit `https://easybone.org/sitemap-index.xml` under **Sitemaps**.
   That file is generated by the build; **do not upload a `sitemap.xml` or
   `sitemap.ts` by hand** — the ones uploaded on 2026-09-23 were Next.js
   templates, which this Astro site cannot run, and they turned CI red until
   they were removed.
2. [Bing Webmaster Tools](https://www.bing.com/webmasters) accepts an import
   from Search Console — a two-minute job worth doing.

**Do not add `rueortho.vercel.app` as a new property.** It has 308-redirected
to `easybone.org` since 2026-09-23, so its sitemap would only list redirects.
If a property for it already exists, leave it in place while Google follows
the redirect; it needs nothing else. (The DNS TXT method was never available
on it — Vercel owns the `vercel.app` zone.)

**If the domain ever changes again (step 5)**, the new address is a separate
property and has to be added and verified the same way. Submit its sitemap,
and leave the old property in place while the redirect settles.
