...
5. **Point the site at the new domain** and push: change `origin` in
   ...
   **Done for `easybone.org` on 2026-09-23**: the domain was attached and
   verified in Vercel, and `origin` now names it. Only the apex was attached;
   add `www.easybone.org` in Vercel too, set to redirect to the apex, so a
   reader who types `www.` does not land on an error.
...
— as a **Domain** property if you can add a DNS TXT record where the domain
is registered (this covers `www.` and `http://` too), otherwise as a URL-prefix
